...
| `0007-migrate-projects.md`(JS) | A' 项目↔对话重构(PR-B / #83，**✅ 已完结 2026-06-15 · 生产无老数据 no-op**) | 老 `chat_sessions` 各成单对话项目:抽 #76 快照工作区写 `projects`、删快照 turn、回填 `project_id`。**需 JS**(parse turns_json),走 owner-only 端点而非纯 SQL。端点已随 **PR-D / #85 收口删除**,runbook 留作历史 |
...
**需 JS 的数据搬迁**(要 parse JSON / 按 sentinel 抽数据,纯 SQL 干不了)曾走一个 **owner-only + owner-scoped 一次性维护端点**(`POST /api/admin/migrate-projects`),逻辑在 `workers/src/migrateProjects.ts`、被 vitest 直接覆盖,默认 dry-run / apply 需 confirm / 先备份。完整步骤见 [`runbooks/0007-migrate-projects.md`](./workers/runbooks/0007-migrate-projects.md)。**该端点 / 脚本 / 备份表已随 PR-D(#85)收口删除**(0007 迁移于 2026-06-15 完结、生产无老数据 no-op,`chat_sessions_a_backup` 由 `0008_drop_migrate_backup.sql` DROP);本段留作历史模式参考(下次需 JS 搬迁仍按此「临时端点 + vitest 覆盖 + dry-run/apply/备份」套路)。
