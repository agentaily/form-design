...
>
> **入口级未登录守卫（PR #90，design-sync `_QB7NM8v`）：** 设计器是**受保护页**——不再「整页渲染 + 仅按动作 `guard()` 拦截」，而是挂在一道**单层客户端入口守卫** `<AuthGate>`（`src/auth-gate.jsx`）之后。进入前先跑**真实** `GET /api/auth/me`（`core/auth.validateSession` → 判别式 `authed | unauthed | error`，区分 401↔5xx/网络，补 `getCurrentUser` fail-soft 到 `null` 合并 401/异常的盲点；无 token 短路为 `unauthed` 不空跑请求），据结果路由：① **已登录** → 实例化设计器（`emailVerified` 驱动顶部**软提醒条**，200 未验证仍进设计器、不硬墙，§23.6）；② **未登录 / 会话失效（401）** → **原地整屏登录视图**（复用 `<SignInScreen>`，登录成功原地重校验、不跳转白屏）；③ **校验异常（5xx / 网络）** → 中性「重试 / 去登录」占位（不把异常误判为未登录）；④ **校验中** → >200ms 才显现品牌占位（极快校验直接落地，不闪 loading）。受保护内容**仅在校验通过后实例化**（零闪烁：未授权用户拿不到设计器任何可见帧或可点控件）。公开 / auth 路由（`/f/:slug`、`/signin`、`/reset-password`、`/verify-email`）**不**受此守卫。守卫为**纯加法**：会话**中途**失效（已进设计器后某动作返回 401）仍由 `DesignerApp` 的 `guard()`/`needLogin` 兜底（bounce `/signin` → 重登 → 守卫原地重跑）；入口守卫保证进设计器即已登录，故 happy path 下 `guard()` 直通。契约见 [`features/auth-gate.feature`](../features/auth-gate.feature)。
>
...
>
> **URL 状态持久化 + 工作区恢复（PR #76，纯前端、无新 D1 / migration）：** 活跃 `designSessionId` 反映进设计器 URL 的 **`?s=<sessionId>` query**（`src/core/router.ts` `readSessionId` / `withSessionId`；与 `/settings/:tab` 路径正交、可共存）：进设计器即把会话规整进 URL（`replaceState`），带 `?s=` 刷新 / 深链 / 分享 → `setActiveDesignSessionId` + 按该 id 走本节的恢复路径；新建 / 切换 `pushState`、浏览器前进后退切回对应会话。**乱序到达防护：** 每次异步会话加载（mount 恢复 + 切换 + popstate）领一个单调 load-sequence token，await 回来后只在「仍是最新加载且活跃会话未变」时才应用——否则**绝不**用过期结果覆盖当前会话（不串会话）。**工作区（右侧表单预览模型）恢复（#76 灰度期机制，仍活着）：** §26 只持久化对话（`turns` + `history`），不存表单模型；回放 tool 调用重建不可靠（字段 `uid` 与消息 id 交错→引用错位）。故把模型快照（`meta` + `fields`）作为**一条合成 turn**（`id = WORKSPACE_SNAPSHOT_ID`、`role: "assistant"`、`kind: "workspace"`）搭进 §26.6 既有的**不透明 `turns_json`**（后端零改、不影响按 `role==="user"` 推的 `title` / `turnCount`），恢复时由 `splitWorkspaceSnapshot` 摘出重建预览、绝不渲染成对话气泡（`src/core/chatSessionClient.ts` `buildWorkspaceSnapshotTurn` / `splitWorkspaceSnapshot`）。行为契约见 `features/url-state-persistence.feature`。**注意（A' 重构 · PR-C #84 已废弃此快照路径）：** 这条「工作区快照骑在 `turns_json`」原是 #76 的灰度期做法，**PR-C 前端切到 A' 后已删除** `WORKSPACE_SNAPSHOT_ID` / `buildWorkspaceSnapshotTurn` / `splitWorkspaceSnapshot`——工作区上提到项目级 `projects` 表（§26.10），前端改由 `src/core/projectClient.ts` 的 `loadProject` / `saveProjectWorkspace` 单独承载（见下方「A' 前端接线」）。迁移期 `splitWorkspaceSnapshot` 的纯逻辑曾被一次性数据迁移脚本（PR-B `migrateProjects.ts`）移植复用、前端契约里已删；该脚本与 `POST /api/admin/migrate-projects` 端点已随 **PR-D（#85）收口一并删除**（生产迁移完结、`chat_sessions_a_backup` 由 `0008` migration DROP），这些快照符号现已全仓无代码引用。
...
> 
> **本批是 PR-A，纯加性 / 灰度：** 0007 migration 只**新建 `projects` 表 + 给 `chat_sessions` 加可空列 + 加索引**，新端点（§26.1 的 projects 四端点 + rename）全部**加性新增**。**旧前端（未切 A'）照常工作**——它不传 `project_id`，会话仍按 `(owner_id, session_id)` 定位（§26.2），工作区仍走上面 #76 的快照路径，本批不动它。**前端真正切到项目级工作区在 PR-C**（按 `project_id` 载工作区、`switchSession` 不动工作区、`persistTurn` 拆两写）；老会话各成单对话项目的一次性数据迁移是 PR-B。本节及 §26.10 只钉**PR-A 的后端结构 + 端点契约**，前端接线 / URL 载体见下方「A' 前端接线（PR-C #84）」。
> 
> **A' 前端接线（PR-C #84，已落地）：** 前端消费 §26.10 端点切到项目模型。① **`projectClient.ts`**（新）= projects 端点的前端 seam（`getOrCreateProjectId`/`setActiveProjectId`/`newProjectId` localStorage helper + `loadProject`/`saveProjectWorkspace`/`listProjects`/`deleteProject`），镜像 `chatSessionClient` 的 session-id helper 结构。② **对话 project-scoped**:`chatSessionClient` 的 `loadChatSession`/`saveChatTurns`/`listChatSessions`/`deleteChatSession` 增 `projectId`（经 **`?projectId=` query** 透传,route 层接上数据层的灰度可选参数,纯加性）+ 新增 `renameChatSession`。③ **App 接线**:`persistTurn` 拆两写（`saveChatTurns(projectId,sessionId,…)` 对话 + `saveProjectWorkspace(projectId,…)` 工作区,**去掉 `editingFormRef` 跳过持久化**——编辑就是进项目、对话照常落库）;`enterProject` 载工作区 + 解析最近对话(`?s=` || 项目最近 || 新建),工作区与对话两路异步**共一个 load-seq token** 防乱序;`switchSession` **只重载对话、工作区不变**;`newChat` 只清对话留工作区;`loadFormForEdit` 按 `formSlug` 反查项目(拿不到则 mint+关联)、载真实最近对话(**去合成「已载入」开场白**)。**裸 URL 加载 resume(cutover)**:首渲项目 id 解析顺序 = `/p/:id` 路径 → localStorage 活跃项目 → **都没有则 mount 异步 `listProjects()` resume 最近项目(updated_at DESC)、owner 无任何项目才 mint 新的**——确保现有 / 刚迁移的 owner 裸开 `/` 落到自己的数据,而非空新项目。④ **URL 路径化 `/p/:projectId?s=:sessionId`**（老板定案,非本节早稿的 `?p=` query）:`router.ts` 加 `PROJECT_PREFIX="/p/"` / `readProjectId` / `projectBasePath`,设置浮层嵌 `/p/:id/settings/:tab`（`matchSettings` / `settingsPath` 认项目前缀）;切对话只改 `?s=`、切项目改 `/p/:id`,popstate 区分两者。⑤ **对话标题可编辑**:`SessionMenu` 行内 rename → `PATCH /api/chat/session/:id`,`ConversationThread` 头部标题 = 当前对话名。行为契约 `features/url-state-persistence.feature` / `features/chat-multi-session.feature` / `features/form-editing.feature`（「编辑态对话照常持久化到项目会话」反转旧「不污染」契约）。
