Feature: 提交校验 状态门与 answers 对 schema 必填校验 POST /api/submit
  ...
  以便我的 D1 主存里不混入「已关闭表单的提交」或「漏填必填项」这类脏数据

  背景：在 §16.5 的 formExists（form 不存在 → 404）之后、§15 写 D1 主存之前，POST /api/submit 有两道门：
  ①状态门——读该 form 的 status，非 published（draft/closed）→ 409 拒收，不读 owner 配置、不落 D1、不同步飞书；
  ②answers 对 schema 校验——按 form 的 fields 校验，required 字段缺失或空值 → 400，不落 D1、不同步飞书。
  两步都在写 D1 之前，确保脏提交绝不落库（也就绝不被后台同步到飞书）。表单经 POST /api/forms 发布出来即
  published，关闭提交通过 PATCH 把 status 改成 closed（见 form-management.feature）。
  注意：架构转向后**未配飞书不再 409**（submit.feature 覆盖），所以这里唯一的 409 语义是「表单未开放提交」——
  最后一个场景验证状态门先于 owner 配置触发、文案不是「未配飞书」。

  Scenario: 向已发布表单提交满足必填的作答正常落 D1 主存
  ...
  Scenario: 向已关闭表单提交返回 409 且不落 D1 不同步飞书
    Given 一个已保存完整飞书凭据的 owner
  ...
  Scenario: 向草稿表单提交返回 409 且不落 D1 不同步飞书
    Given 一个已保存完整飞书凭据的 owner
    And 一份状态为 draft 的表单
    When 答题者带着该表单的 slug 向 /api/submit 提交一份作答
    Then 响应状态码为 409
  ...
  Scenario: 漏填必填字段返回 400 且不落 D1 不同步飞书
    Given 一个已保存完整飞书凭据的 owner
