Feature: CORS 跨源访问控制 覆盖所有 /api/* 端点
  ...
  写请求，导致工作区/对话「刷新即丢」——「继续编辑已发布表单 → 刷新 → 工作台空」即此根因。）
  ...
  Scenario: PUT 写端点（项目工作区 / 对话 / 显示名）的预检放行 PUT
    ...
    # 工作区/对话刷新即丢（「继续编辑已发布表单+刷新工作台空」回归根因）。
    Given 一个 Origin 为生产前端域名的请求
    When 浏览器对 PUT /api/projects/:projectId 发起请求方法为 PUT 的 OPTIONS 预检
    Then 响应状态码为 2xx
    And 该预检没有被鉴权拦成 401
    And 响应头 Access-Control-Allow-Origin 回显该白名单来源
    And 响应头 Access-Control-Allow-Methods 含 PUT
