...
- **下一步候选（待办）**：飞书增强剩余（改字段标签同步 / 字段 id↔列稳定映射、`listBitableColumns` >100 列分页、list 故障时降级到无类型写值而非 502、`/submissions` 按 `formSlug` 过滤）、数据后台增强（CSV 导出 / 分页 / 统计）、**限流加固**（小时窗 429 外环补测、双窗 `Retry-After` 取较大值;**Durable Objects 替 KV** 求强一致、精确无漏限流；**全局 Resend 日发送封顶** —— 无论多少 IP，当天总发送够 N 封即停，直接护住 100/天那条线，补 per-IP 挡不住分布式的缺口）。
