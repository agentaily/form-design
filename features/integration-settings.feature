...
# + §17（owner-only 鉴权，缺/坏 token → 401）。本 feature 描述前端「集成设置 modal」的可观察行为，
# 不重述后端字段加密 / 掩码算法（那是 Worker 内部约定）。前端契约桩见 src/core/configClient.ts，
# 组件见 src/settings.jsx，入口挂载见 src/App.jsx。
Feature: 集成设置 modal · owner 配置 DeepSeek 与飞书
  作为表单作者(owner)
  我想在一个集成设置弹窗里连接自己的 DeepSeek key 与飞书多维表格
  ...
  Scenario: 打开设置拉取并回显已保存配置（密钥掩码）
    ...
    When owner 打开集成设置
    Then 弹窗用掩码值回显 DeepSeek key 与飞书 app_secret
  ...
  Scenario: 不修改的密钥字段不会被覆盖
    Given owner 已登录并打开集成设置
    And 弹窗回显着 DeepSeek key 与飞书 app_secret 的掩码值
  ...
  Scenario: 测试连接逐条显示 DeepSeek 与飞书结果
    ...
    Then 弹窗把 DeepSeek 标记为连通
    And 弹窗把飞书标记为不可连通并显示其说明

  Scenario: 连不通是正常结果而非报错
    Given owner 已登录并打开集成设置
    When owner 点击测试连接
    And 后端返回两块都未配置
    Then 弹窗逐条显示两块均不可连通及其说明
    And 弹窗不显示请求失败的报错
