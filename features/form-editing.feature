...
Feature: 表单编辑入口（载回设计器继续编辑）
  ...
  # —— 编辑对话就是项目的对话（A' 反转，§26.10 + 重构方案 §4.4）——
  # 旧契约把编辑态对话当临时的、跳过 §26 持久化以免覆盖设计对话。A' 把这反转过来：「继续编辑」=
  # 进入该表单对应的项目，编辑态对话就是该项目下的一条会话，照常持久化到项目的会话行。这修了
  # 「继续编辑后刷新对话丢」的回归（旧 editingFormRef 跳过持久化把每个编辑回合都丢了）。工作区
  # （正在编辑的表单 meta+字段）也落到项目行（与会话 turns 解耦的两次写）。

  Scenario: 编辑态的对话照常持久化到项目的会话
    Given owner 已把一份已发布表单载回设计器编辑
    When owner 在编辑态发生一轮对话改动
    Then 这轮编辑对话写进该项目下的会话持久化
    And 编辑改动的工作区也落到该项目行
